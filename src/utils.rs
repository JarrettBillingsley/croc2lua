...
macro_rules! croc_tag {
    ($i:expr, $tag:expr) => (
        eat_croc_sep!($i, tag!($tag))
    );
}

// Eats a preceding croc separator, i.e., whitespace and/or a comment till the end of the line
macro_rules! eat_croc_sep {
    ($i:expr, $submac:ident!( $($args:tt)* )) => ({
        use utils::croc_sep;
        complete!(
            $i,
            preceded!(croc_sep, $submac!($($args)*))
        )
    });
}
...
named!(pub croc_sep,
    recognize_many0!(
        alt!(take_while1!(is_whitespace) | comment)
    )
);
